	...
	for (i = 0; i < PAGE_NUMBER; i++)//遍历虚拟内存
	{
		...
		if (virMemory.virTable[i][0] == _pid)
		{
			...
			for (j = 0; j < FRAME_NUMBER; j++)//遍历物理内存
			{
				...
				{
					DeletePage(i, pageTable[_pid]);//将进程对应的页表删除
			...
			allocVirMemory -= virMemory.virTable[i][0];//已分配虚拟内存减去释放内存
			//初始化虚拟内存
...
int AllocVirMemory(int _pid, int _size)
{
	...
	for (i = 0; i < PAGE_NUMBER; i++)//遍历虚拟内存
	{
		...
		if (virMemory.virTable[i][0] == -1)
		{
			...
			InsertPage(i, tmpTable);//在该页帧对应表插入新的关联
...
Table LRU(int _virID, Table _table)
{
	...
	else if (phyID != -1)
	{
		...
		ModifyPage(_virID, phyID, _table, 1);//将页调入物理内存
	...
	else
	{
		...
		pageFault++;//增加缺页次数
		ModifyPage(reVirID, 0, tmpTable, -1);//将置换出的页调出物理内存
		scheQueue.pop_front();//置换出的页出调度队列
		scheQueue.push_back(_virID);	//将置换进的页添加到调度队列末尾
		ModifyPage(_virID, rePhyID, _table, 1);//将置换出的页调入物理内存
